...
class Agent extends CommonObject
{
	...
	{
		// Vérification si agent déjà existant
		$existingAgent = $this->fetch('','',' AND fk_user='.$this->fk_user);

		if($existingAgent) setEventMessage('Cet utilisateur possède déjà un agent lié.','errors');
		elseif(!$this->fk_annee_scolaire) setEventMessage('Veuillez choisir une année scolaire.','errors');
		else
		{
			// Récupération des infos de l'utilisateur cible
			$userClass = new User($this->db);
			$userClass->fetch($this->fk_user);

			if((strtolower($userClass->lastname) !== strtolower($this->nom)) || (strtolower($userClass->firstname) !== strtolower($this->prenom)))
			{
